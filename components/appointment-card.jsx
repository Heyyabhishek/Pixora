...
export function AppointmentCard({
  ...
  // Handle save notes (creator only)
  ...
  // ⭐ FIXED: Use correct Prisma relation names
  const otherParty =
    userRole === "CREATOR"
      ? appointment.User_Appointment_clientIdToUser
      : appointment.User_Appointment_creatorIdToUser;

  // Safety check - if otherParty is undefined, show a placeholder
  if (!otherParty) {
    return (
      <Card className="border-red-900/20">
        <CardContent className="p-4">
          <p className="text-red-400">
            Error: Unable to load appointment details. Missing user information.
          </p>
        </CardContent>
      </Card>
    );
  ...
  const otherPartyIcon = userRole === "CREATOR" ? <User /> : <Camera />;
