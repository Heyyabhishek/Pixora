...
/**
 * Book a new appointment with a creator
 */
export async function bookAppointment(formData) {
  ...
  try {
    ...
    if (!creatorId || !startTime || !endTime) {
      throw new Error("Creator, start time, and end time are required");
    }

    // Check if the creator exists and is verified
    ...
    if (!creator) {
      throw new Error("Creator not found or not verified");
    }
    ...
    // Deduct credits from client and add to creator
    const result = await deductCreditsForAppointment(client.id, creator.id);
 ...
 * This will be called when either creator or client is about to join the call
 */
export async function generateVideoToken(formData) {
  ...
  try {
    ...
    const token = vonage.video.generateClientToken(appointment.videoSessionId, {
      role: "publisher", // Both creator and client can publish streams
 ...
 */
export async function getAvailableTimeSlots(creatorId) {
  try {
    // Validate creator existence and verification
    const creator = await db.user.findFirst({
      where: {
        id: creatorId,
        role: "CREATOR",
    ...
    if (!availability) {
      throw new Error("No availability set by creator");
    }
    ...
    // Fetch existing appointments for the creator over the next 4 days
